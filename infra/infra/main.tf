...
module "security_group" {
  ...
  ec2_sg_name_for_python_api = "${var.my_env}-SG for EC2 for enabling port 5000"
}
...
module "ec2" {
  ...
  tag_name                 = "${var.my_env}-Ubuntu Linux EC2"
...
module "lb_target_group" {
  ...
  lb_target_group_name     = "${var.my_env}-dev-proj-1-lb-target-group"
...
module "alb" {
  ...
  lb_name                   = "${var.my_env}-dev-proj-1-alb"
  is_external               = false
  lb_type                   = "application"
  sg_enable_ssh_https       = module.security_group.sg_ec2_sg_ssh_http_id
  subnet_ids                = tolist(module.networking.dev_proj_1_public_subnets)
  tag_name                  = "${var.my_env}-dev-proj-1-alb"
...
module "rds_db_instance" {
  source               = "../rds"
  db_subnet_group_name = "${var.my_env}-dev_proj_1_rds_subnet_group"
